...
def get_df_players_on_ice_per_sec(game_id: int, df_penalties: pd.DataFrame, df_goals: pd.DataFrame) -> pd.DataFrame:
...
def add_players_on_ice_per_sec(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for side in ["home", "away"]:
        df.loc[mask, f"{side}_players_on_ice"] = (
            df.loc[mask, f"{side}_players_in_box"].mul(-1).add(MAX_PLAYERS_ON_ICE).clip(lower=MIN_PLAYERS_ON_ICE)
        )
...
def get_strength(df: pd.DataFrame) -> np.array:
    ...
    return np.where(
        ...
        (df.away_players_on_ice.astype(int).astype(str) + "v" + df.home_players_on_ice.astype(int).astype(str)),
        (df.home_players_on_ice.astype(int).astype(str) + "v" + df.away_players_on_ice.astype(int).astype(str)),
    )
